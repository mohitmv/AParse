...
struct LexerScope: public aparse::AdvanceLexerScopeBase {
...
using MyLexer = aparse::AdvanceLexer<LexerScope>;

void BuildLexer(MyLexer* lexer) {
  using Rule = MyLexer::Rule;
  using TokenType = LexerScope::TokenType;
  MyLexer::LexerRules lexer_rules;
    ...
    Rule("<atom> ::= NUMBER | OPEN_B1 <main> CLOSE_B1 ")
      .Action([](ParserScope* scope, Expression* output) {
        if (scope->Exists(0)) {
          ...
          output->value = std::stoi(scope->tokens->at(scope->AlphabetIndex(0)).second);
        } else {
...
void BuildParser(Parser* parser) {
  ...
  builder.Build(MyGrammar(), parser);
}
...
class ParserBuilderIntegrationTest : public ::testing::Test {
 ...
 protected:
  ...
  Expression Parse(const string& content, ParserInstance& p) {
    auto lexer = lexer_main.CreateInstance();
    lexer.Scope().tokens.clear();
    lexer.Scope().content = &content;
    for (auto c: content) {
...
TEST_F(ParserBuilderIntegrationTest, Basic) {
  ...
  EXPECT_EQ(Parse(" 433 + 88 *( 445 + 99 + 44 * 8 +(22 + 5)+( 3+ 33))+544").Eval(), 85369);
}

TEST_F(ParserBuilderIntegrationTest, DISABLED_ImportExport) {
  // ParserBuilder builder;
  // std::size_t grammar_hash = MyGrammar().GetCheckSum();
  // string parser_export = builder.Export(parser_main, grammar_hash);
  // Parser p1;
  // EXPECT_TRUE(builder.Import(parser_export, grammar_hash, &p1));
  ...
  // EXPECT_EQ(parser_export.size(), p1.Export().size());
}
